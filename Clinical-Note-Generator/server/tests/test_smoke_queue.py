...
def test_queue_rejects_legacy_audio_job_types(client):
    from server.app import app
    from server.core.dependencies import get_current_user
    from server.models.user import User
    ...
    fake_user = User(
        id=uuid.uuid4(),
        email="queue-audio-reject@example.com",
        hashed_password="x",
        is_active=True,
        is_approved=True,
    )
    ...
    for job_type in ("transcribe", "asr", "asr_recording", "asr_segment"):
        resp = client.post(
            "/api/queue",
            data={"type": job_type},
            files={"file": ("clip.webm", b"not-a-real-clip", "audio/webm")},
        )
