...
class _FakeQAGenerator:
    """Separate stub for qa_routes.get_simple_note_generator() so the note
    and QA assertions below can't collide on the same collect_completion
    return value."""
...
def test_smoke_note_ocr_qa(client, monkeypatch):
    ...
    monkeypatch.setattr(qa_routes, "get_simple_note_generator", lambda *_a, **_k: _FakeQAGenerator())
    ...
    assert "SMOKE_NOTE_TEXT" in note_resp.text
