...
DEFAULT_PREPROCESSING_CONFIG = {
    ...
    "truncation": {
        "enabled": True,
        "prior_visits_budget_tokens": DEFAULT_SECTION_BUDGET_TOKENS,
        "labs_imaging_other_budget_tokens": DEFAULT_SECTION_BUDGET_TOKENS,
        "current_encounter_budget_tokens": DEFAULT_SECTION_BUDGET_TOKENS,
    },
}


def effective_preprocessing_config(cfg: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Merge config.json preprocessing with safe defaults (cleaning on, 24k section truncation)."""
    base = deepcopy(DEFAULT_PREPROCESSING_CONFIG)
    user = ((cfg or {}).get("preprocessing")) or {}
    if not isinstance(user, dict):
        return base

    if isinstance(user.get("steps"), dict):
        base["steps"].update(user["steps"])

    trunc_user = user.get("truncation") if isinstance(user.get("truncation"), dict) else {}
    for key in (
        "prior_visits_budget_tokens",
        "labs_imaging_other_budget_tokens",
        "current_encounter_budget_tokens",
    ):
        if key in trunc_user:
            try:
