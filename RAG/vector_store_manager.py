...
def _load_npz(fp: Path) -> Tuple[List[str], List[str], List[List[float]], List[Dict[str, Any]]]:
    # SECURITY: allow_pickle=True is intentional here. The .npz files contain structured
    # arrays (ids, texts, embeddings, metadatas) with object-dtype fields that inherently
    # require pickle for serialization. This is safe because:
    #   1. Files are generated locally by the RAG pipeline — not user-uploaded or externally
    #      sourced. The pipeline writes its own index from trusted clinical guidelines.
    #   2. The .npz index is never served over HTTP or exposed to untrusted input.
    #   3. The data originates from curated medical guideline documents, not arbitrary
    #      user content.
    # If the pipeline is ever extended to accept external .npz files, migrate to a
    # pickle-free format (e.g., separate JSONL for metadata + .npy for embeddings).
...
class VectorStoreManager:
    ...
    def _flush_batch(
        ...
        # Multiple chunk files can emit the same chunk id; Chroma requires unique IDs per upsert batch.
        if len(ids) != len(set(ids)):
            ...
            metas_clean = [metas_clean[i] for i in keep]
