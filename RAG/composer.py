...
def build_cited_opinion(query: str, hits: List[Dict[str, Any]]) -> str:
    ...
    with meter:
        ...
        # Build cited content with inline citations
        cited_content = []
        for i, hit in enumerate(top, start=1):
            text = hit.get("text", "")
            summary = hit.get("summary", text)
            if summary:
                cited_content.append(f"[{i}] {summary}")
        ...
        body: List[str] = [
            ...
            "",
            "Clinical Impression:",
            "Based on the evidence above, provide a structured clinical opinion with:",
            ...
            "",
            "Citations:",
            "Each statement above must be supported by at least one reference [N].",
            "If evidence is conflicting, note the discrepancy and cite both sources.",
            "",
            "References:",
            refs,
        ]
