    ...
    if path.suffix.lower() == ".jsonl":
        with path.open("r", encoding="utf-8") as f:
            for line_no, line in enumerate(f, start=1):
                # Some upstream tools may emit BOM or non-JSON sentinel lines.
                ...
                try:
                    ...
                    # Best-effort robustness: skip bad lines but keep the run going.
                    # Weekly runs should not fail due to a single malformed record.
                    sample = line[:120].encode("unicode_escape", errors="backslashreplace").decode("ascii", errors="ignore")
                    print(f"WARNING: Skipping invalid JSONL line {line_no} in {path.name}: {sample}")
                    continue
                if isinstance(obj, dict):
                    yield obj
                else:
                    # If a JSONL line decodes to a non-dict (rare), skip it.
                    print(f"WARNING: Skipping non-dict JSONL line {line_no} in {path.name}: {type(obj)}")
    ...
    with out_path.open("w", encoding="utf-8") as out:
        for i, obj in enumerate(iter_input_records(path)):
            ...
            for j, (ch, heading) in enumerate(parts):
                ...
                strength, quality = extract_grade(body)
                grade_meta = {}
                if strength:
