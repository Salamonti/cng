window.WORKSPACE_PAGE_TYPE = 'main';
                            ...
                            if (mode === 'chunk' && caps && caps.chunk_asr_enabled && window.AsrChunkPipeline && typeof audioHandler.setChunkPipeline === 'function') {
                                ...
                                const chunkPipeline = new window.AsrChunkPipeline({
                                    ...
                                    fetchMergedFn: function () {
                                        return fetchAsrPipelineMerged(eidRec, app._asrChunkSessionId);
                                    },
                                    ...
                                    onTranscript: function (merged) {
                                        const tx = document.getElementById('transcriptionDisplay');
                                        const base = app.asrBaselineBeforeRecording != null ? app.asrBaselineBeforeRecording : '';
                                        if (tx && merged) {
        ...
        async function applyEncounterTranscriptText(encounterId, text) {
            ...
            const tx = document.getElementById('transcriptionDisplay');
            if (tx) {
                const useBaseline = app && app.asrBaselineBeforeRecording != null && app.isListening;
        ...
        async function fetchAsrPipelineMerged(encounterId, recordingSessionId) {
            const token = getAuthToken();
            const eid = String(encounterId || (app && app.activeEncounterId) || '');
            const sess = String(recordingSessionId || '');
            if (!token || !eid || !sess) return '';
            const qs = '?recording_session_id=' + encodeURIComponent(sess);
            const pr = await apiFetch('/asr/segments/encounter/' + encodeURIComponent(eid) + '/pipeline' + qs, {
                headers: { 'Authorization': 'Bearer ' + token },
            });
            if (!pr.ok) return '';
            const pipeline = await pr.json();
        ...
        async function drainAsrChunkPipeline(encounterId, recordingSessionId) {
            ...
            let url = '/asr/segments/encounter/' + encodeURIComponent(eid) + '/drain';
            const params = [];
            if (sess) params.push('recording_session_id=' + encodeURIComponent(sess));
            ...
            const drainResp = await apiFetch(url, {
                method: 'POST',
                headers: { 'Authorization': 'Bearer ' + token },
            });
            if (!drainResp.ok) {
                throw new Error('Chunk drain failed: ' + drainResp.status);
            }
            let pipeline = await drainResp.json();
            const pollQs = sess ? ('?recording_session_id=' + encodeURIComponent(sess)) : '';
            for (let i = 0; i < 120 && pipeline && pipeline.state === 'processing'; i++) {
                await new Promise(function (r) { setTimeout(r, 500); });
                const pollResp = await apiFetch('/asr/segments/encounter/' + encodeURIComponent(eid) + '/pipeline' + pollQs, {
                    headers: { 'Authorization': 'Bearer ' + token },
                });
                if (pollResp.ok) pipeline = await pollResp.json();
                else break;
            }
            return pipeline;
        }
        ...
        function renderRagContent(content) {
            ...
            if (refs.length) {
                ...
                refs.forEach(r => {
                    ...
                    const src = safeStr(r.source || r._kind || '').trim() ? ` [${r.source || r._kind}]` : '';
                    if (url) {
        ...
        function renderStructuredComment(structured, rawComment) {
            ...
            if (diffs.length) {
                const sect = createCollapsibleSection('🔬 Differential Diagnosis', diffs.map(d => ({
                    ...
                    body: (d.rationale || '') + (d.evidence_source ? ' <span class="ev-src">' + escapeHtml(d.evidence_source) + '</span>' : ''),
            ...
            if (workup.length) {
                const sect = createCollapsibleSection('🔬 Recommended Workup', workup.map(w => ({
                    ...
                    body: (w.rationale || '') + (w.evidence_source ? ' <span class="ev-src">' + escapeHtml(w.evidence_source) + '</span>' : ''),
            ...
            if (mgmt.length) {
                const sect = createCollapsibleSection('💊 Management Adjustments', mgmt.map(m => ({
                    ...
                    body: (m.rationale || '') + (m.evidence_source ? ' <span class="ev-src">' + escapeHtml(m.evidence_source) + '</span>' : ''),
            ...
            if (Object.keys(ev).length) {
                ...
                row.innerHTML =
                    '<div><span class="ev-summary-label">Sources</span><span class="ev-summary-val">' + (ev.sources_consulted || 0) + '</span></div>' +
                    '<div><span class="ev-summary-label">Quality</span><span class="ev-summary-val">' + evidenceQualityLabel(ev.evidence_quality) + '</span></div>';
                if (ev.limitations) {
        ...
        function createCollapsibleSection(title, items) {
            ...
            items.forEach(item => {
                ...
                if (item.body) {
                    content.innerHTML += '<br>' + escapeHtml(item.body);
                }
        ...
        // Generate patient materials via API
        async function generatePatientMaterials() {
            const genId = window.lastGenerationId;
            if (!genId) return;

            const noteTextarea = document.getElementById('generatedNote');
            if (!noteTextarea || !noteTextarea.value.trim()) {
                showToast('Error', 'No note to generate materials from.', 'error');
                return;
            }

            const state = ensurePatientMaterialsState();
            ...
            try {
                const response = await apiFetch('/patient-materials/generate-all', {
                    method: 'POST',
                ...
                // Render materials
                renderPatientMaterials(data);

                // Save state
                saveUiState();

            } catch (error) {
                console.error('Patient materials generation failed:', error);
                state.status = 'error';
                showToast('Error', 'Failed to generate patient materials: ' + error.message, 'error');
            }
        }
        ...
        // Render patient materials
        function renderPatientMaterials(materials) {
            Object.keys(materials).forEach(function(matType) {
                const mat = materials[matType];
                const capitalized = matType.charAt(0).toUpperCase() + matType.slice(1);
                const contentEl = document.getElementById('pmContent' + capitalized);
                const loadingEl = document.getElementById('pmLoading' + capitalized);
                const disclaimerEl = document.getElementById('pmDisclaimer' + capitalized);
                const flagsEl = document.getElementById('pmSafetyFlags' + capitalized);

                if (loadingEl) {
                    loadingEl.classList.add('hidden');
                }
                if (contentEl && mat.content) {
                    contentEl.innerHTML = renderMarkdownSimple(mat.content);
                    contentEl.classList.remove('hidden');
                }
                if (disclaimerEl && mat.disclaimer) {
                    disclaimerEl.textContent = mat.disclaimer;
                    disclaimerEl.classList.remove('hidden');
                }
                if (flagsEl && mat.safety_flags && mat.safety_flags.length > 0) {
                    flagsEl.innerHTML = '<h5>Safety Notes:</h5><ul>' +
                        mat.safety_flags.map(function(f) { return '<li>' + escapeHtml(f) + '</li>'; }).join('') +
                        '</ul>';
