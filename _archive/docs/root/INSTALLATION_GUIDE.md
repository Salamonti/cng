...
| `ADMIN_BOOTSTRAP_EMAIL` | (optional) | If set with `ADMIN_BOOTSTRAP_PASSWORD`, creates **one** approved admin user on startup when that email does not exist (operator-only; never commit real values) |
| `ADMIN_BOOTSTRAP_PASSWORD` | (optional) | Password for bootstrap admin (use only with `ADMIN_BOOTSTRAP_EMAIL`) |
...
**Operator checklist (NSSM rows, venv, `ADMIN_*` gates, Whisper CLI):** see **[`docs/OPERATOR_RUNBOOK_WINDOWS.md`](docs/OPERATOR_RUNBOOK_WINDOWS.md)**.  
**Which planning doc wins (archived snapshot):** **[`docs/planning-archive/ROADMAP_AUTHORITY.md`](docs/planning-archive/ROADMAP_AUTHORITY.md)**. For the next pass, start from **[`docs/planning-archive/FUTURE_PLAN_BACKLOG.md`](docs/planning-archive/FUTURE_PLAN_BACKLOG.md)**.
